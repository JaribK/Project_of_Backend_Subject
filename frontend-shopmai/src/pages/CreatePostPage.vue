                ...
                <div id="create-box"
                    ...
                    <div id="writen" class=" h-[764px] w-[957px] grid">
                        ...
                        <div id="info" class=" w-full h-[630px] grid place-items-center ">
                            <div id="inboxinfo" class="w-[957px] h-[540px]">
                                ...
                                <div id="box-post4" class=" w-full h-[68px] flex">
                                    ...
                                    <div id="attribute-box" class="flex-2 w-[630px] border-b-2 border-[#5F5F5F]">
                                        <div class="shadow-2xl">
                                            <input type="file" @change="handleFileChange" ref="file"
                                                accept=".png, .jpg, .jpeg"
                                                class="w-full text-black text-lg bg-gray-100 roundend-lg file:cursor-pointer cursor-pointer file:border-0 file:py-3 file:px-4 file:mr-4 file:bg-gray-800 file:hover:bg-gray-700 file:text-white rounded-lg" />
                                        </div>
                                    </div>
                        ...
                        <div id="next" class="justify-self-center pb-10 space-x-32">
                            ...
                            <button type="button" class="btn btn-error text-white text-[20px] w-[173px] h-[48px]" @click="cancelpostPost">ยกเลิก</button>
                        </div>
                    </div>
                </div>
            </div>
                ...
                if (this.post_thumbnail) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            axios.post(host + 'api/posts/', fd, {
                                headers: {
                                    'Content-Type': 'multipart/form-data'
                                },
                            }).then(response => {
                                this.$router.push(`/post/${response.data.id}/create-product`)
                            })
                        }
                    })
                } else {
